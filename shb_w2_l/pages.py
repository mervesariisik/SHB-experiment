...
class SHB_Example3_1(Page):
    ...
    def error_message(self, values):
        if values['ex3_s_n_5'] is True or values['ex3_s_n_6'] is True or values['ex3_s_n_7'] is True or \
                values['ex3_s_n_8'] is True or values['ex3_s_n_9'] is True or values['ex3_s_n_10'] is False:
            return "The selected set of secret numbers is wrong! Please check again, remember to use " \
                   "the table."
...
class Observer_Decision(Page):
    def is_displayed(self):
        return self.player.role == 'Observer'

    form_model = 'group'
    form_fields = ['S_disclose']

    def vars_for_template(self):
        return {
            'S_disclose_label': 'Do you want to send the Message {} to the Guesser?'.format(self.group.S_signal)
        }
    ...
    def before_next_page(self):
        ...
        if timeout_happened:  # we use "timeout_happened" to decide what value gets submitted on user's behalf
            ...
            if coin_toss == 1:
                p.group.S_disclose = True
            elif coin_toss == 2:
                p.group.S_disclose = False

            print("<Time Out on Page:'Observer_Decision'>", "participant label:", p.participant.label,
                  "participant id in sesh:", p.participant.id_in_session, "round number:", self.round_number,
                  "automated random answer:", p.group.S_disclose)


class Round1_Wait_for_Observer(WaitPage):  # NOTE: This is not necessary on W1s & W3.1 as sender doesn't make any decisions...
    def is_displayed(self):
        return self.round_number == 1

    title_text = 'Your role: Guesser'
    body_text = "Please wait for the Observer to make this round's decision."


class Wait_for_Other_Player(WaitPage):  # NOTE: This is not necessary on W1s & W3.1 as sender doesn't make any decisions...
    def is_displayed(self):
        return self.round_number != 1

    template_name = 'shb_w2_l/Wait_for_Other_Player_template.html'
...
class SHB_RoundResultsWaitPage(WaitPage): # Guesser doesn't really see this page as they proceed immediately after he
    # also 'enters' the this waitpage upon choosing a guess
    #def is_displayed(self):
        #return self.round_number == 1
    ...
    title_text = 'Please Wait'
    body_text = "Guesser is deciding on this round's guess."
...
#class SHB_RoundResultsWaitPage(WaitPage): # Guesser doesn't really see this page as they proceed immediately after he
    ...
    #title_text = 'Please Wait for the Guesser'
    #body_text = "Note that waiting briefly between rounds is normal as " \
                #"you are randomly matched with Guessers each round. The Guesser may be finishing a " \
                #"previous round or making this round's decision."
