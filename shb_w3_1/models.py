...
TREATMENT: W3.1 (Mandatory Disclosure, Right & Left Skewed Error)
APP: Mandatory Disclosure Game
"""


class Constants(BaseConstants):
    name_in_url = 'shb_w3_1'
    players_per_group = 2
    num_rounds = 15
    K = 0.5
    types = [5, 6, 7, 8, 9, 10]
    ...
    def creating_session(self):
        ...
        if self.round_number == 1:
            ...
            for p in self.get_players():
                if p.participant.id_in_session in list(range(1, self.session.num_participants + 1, 4)):
                ...
                elif p.participant.id_in_session % 2 != 0:
                    p.participant.vars['S_dist'] = 'Forest'
                    print("player", p, "is", p.role, "id_in_session is:", p.participant.id_in_session, "S_dist is",
                          p.participant.vars['S_dist'])
        ...
        for g in self.get_groups():
            ...
            if g.S_dist == 'Forest':  # left skewed error distribution
                left_error = random.choices(Constants.errors, cum_weights=(Constants.K_inv, 2 * Constants.K_inv,
                                                                           3 * Constants.K_inv, 4 * Constants.K_inv,
                                                                           5 * Constants.K_inv, 6 * Constants.K_inv,
                                                                           7 * Constants.K_inv, 8 * Constants.K_inv,
                                                                           9 * Constants.K_inv, 10 * Constants.K_inv,
                                                                           10 * Constants.K_inv + Constants.K), k=1)[0]
                # random.choices creates a list, indexing it to extract an integer value
                g.S_error = left_error
            else:  # right skewed error distribution
